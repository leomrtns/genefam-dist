...
typedef struct splitspr_struct* splitspr;
...
struct splitset_struct
{
  ...
  bool match; /*! \brief do we want to calculate the minimum cost assignment? (used in first round of SPR/RF calculation  */
};

struct splitspr_struct
{
  int gsize, spsize, asize, dsize;
  int spr, spr_extra, rf, hdist; /*! \brief spr, extra prunes for spr, rf distances and hdist=assignment cost */
  int n_g, n_s, n_agree, n_disagree; 
  bipartition *gene, *gs1, *gs2, *sp, *agree, *disagree; /* gene points to vec[0], gs1 points to vec[x] */
  bipartition prune, count_sp;
  hungarian h; /* hungarian method for solving the assignment between edges */
  bool match;  /*! \brief do we want to calculate the minimum cost assignment? is this an RF struct (and not SPR)?  */
};
...
/*! \brief Allocate space for splitset structure for RF calculation (also allocates aux vectors) */
splitset new_splitset_RF (int nleaves);
...
/*! \brief Splitset structure for RF calculation for gene/species tree mapping */
splitset create_splitset_RF_genespecies (topology gene, topology species);
...
int RF_gene_species (topology gene, topology species, splitset split);
/*! \brief h distance (edge disagreement assignment cost) between unrooted gene and species trees (leafset mapping) using RF structure */
int RF_gene_species_hdist (topology gene, topology species, splitset split);
